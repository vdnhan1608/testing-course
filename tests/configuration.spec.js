...
for (let webDriver of webDrivers) {
	describe(`Configuration Script ${webDriver}`, function () {
		let driver;
		this.timeout(100000);
		...
		testcases.forEach(function (testcase) {
			it(`Test case: ${testcase.description}`, async function () {
				try {
					...
					// I will find ways to sendKeys to this later
					...
					if (error.name === 'TimeoutError')
						assert.fail(`Fail test case: ${testcase.description}`);
					else assert.fail('An error occur: ', error); // Any wrong syntax this error will be thrown
				}
			});
		});
	});
